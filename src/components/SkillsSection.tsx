...
function SkillTag({ skill, index }: { skill: string; index: number }) {
  return (
    <motion.div
      ...
      className="px-3 py-1 bg-muted/80 backdrop-blur-sm rounded-md text-sm border border-blue-500/20 shadow-sm"
    >
      {skill}
    </motion.div>
  );
      ...
      <div className="container max-w-4xl mx-auto px-6 md:px-4">
        ...
        >
          ...
          <motion.div variants={skillCategoryVariants}>
            <GlassCard className="p-4">
              <h3 className="text-lg font-medium mb-3 text-center md:text-left flex items-center">
                <span className="mr-2 text-xl">⛓️</span> Blockchain Development
              </h3>
              <div className="flex flex-wrap gap-2 justify-center md:justify-start">
                {skills.blockchainDevelopment.map((skill, index) => (
                  <SkillTag key={skill} skill={skill} index={index} />
                ))}
              </div>
            </GlassCard>
